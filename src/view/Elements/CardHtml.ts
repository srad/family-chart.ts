import { miniTreeSvgIcon, personSvgIcon, plusSvgIcon } from './Card.icons';
import { select } from 'd3';
import { CardDim } from '../../Cards/CardBase';

export enum CardStyle {
  Default, ImageCircle, ImageRect, ImageCircleRect, Rect,
}

export type CardProps<T> = {
  empty_card_label: string;
  onCardUpdate: (e: Event, d: Element) => void;
  onCardClick: (e: Event, d: Element) => void;
  onCardMouseenter: (e: Event, d: Element) => void;
  onCardMouseleave: (e: Event, d: Element) => void;
  style: CardStyle;
  mini_tree: boolean;
  card_display: ((data: T) => void)[];
  card_dim: CardDim;
};

export function createCardHtml<T>(props: CardProps<T>) {
  const cardInner = props.style === CardStyle.Default ? cardInnerDefault
    : props.style === CardStyle.ImageCircleRect ? cardInnerImageCircleRect
      : props.style === CardStyle.ImageCircle ? cardInnerImageCircle
        : props.style === CardStyle.ImageRect ? cardInnerImageRect
          : props.style === CardStyle.Rect ? cardInnerRect
            : cardInnerDefault;
  ...
  return function (d) {
    ...
    this.querySelector('.card')
      .addEventListener('click', e => props.onCardClick(e, d));
    ...
    if (props.onCardMouseenter) {
      select(this)
        .select('.card')
        .on('mouseenter', e => props.onCardMouseenter(e, d));
    }
    if (props.onCardMouseleave) {
      select(this)
        .select('.card')
        .on('mouseleave', e => props.onCardMouseleave(e, d));
    }
  };
