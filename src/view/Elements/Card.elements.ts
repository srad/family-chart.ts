...
import { CardDim, Store } from "../../Cards/CardBase";
import { Form } from "../../CreateTree/form";
...
function cardEditIcon(d: { data: Person }, props: { cardEditForm: Form, card_dim: CardDim, store: Store<any> }) {
  if (d.data.to_add) {
    return;
  }
  const card_dim = props.card_dim;
  ...
  g.on("click", (e) => {
    e.stopPropagation();
    cardEdit(props.store, { datum: d, cardEditForm: props.cardEditForm });
  });
