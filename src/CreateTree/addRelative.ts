...
export default (store: Store, cancelCallback?: () => void, onSubmitCallback?: () => void) => {
  return new AddRelative(store, cancelCallback, onSubmitCallback);
}

export type RelDefaults = {
  father: string;
  mother: string;
  spouse: string;
  son: string;
...
export class AddRelative {
  ...
  constructor(store: Store, cancelCallback?: () => void, onSubmitCallback?: () => void) {
    this.store = store;
